...
**Intent:** Add Define: the first row of the AI toolbar, right under the highlight colors, shown only when the selection is one word or one phrase, on every document the reader draws (articles, PDFs, transcripts, formalized articles, slides, sheets, converted handwritten pages, cores in the collapsed view); it shows what the word means in its sentence.

**Files:**
- `prisma/schema.prisma`, `prisma/migrations/20260925100000_define_derivation/migration.sql` — `DEFINE` in `DerivationType`. The pipeline's per-type config is keyed on the enum, and the ToolRating comment names the new tool.
...
- `scripts/qa/mock-kimi.mjs`, `scripts/qa/ui-define.mjs` — the mock's Define answer, and the browser check (34 checks: the route, an article, a phrase, a sentence, a sentence end, a key term, a core, a transcript, slides, a sheet, Chinese, a touch screen).
