...
- `src/lib/gateway.ts`: `typesafe` provider; `litellm/config.yaml`: the `/typesafe/systemone` pass-through to OpenRouter; `litellm/.env.example`, `litellm/README.md`: `TYPESAFE_API_KEY` on the gateway host. `src/lib/jev.ts` calls the pass-through with the app key under the gateway, stands down five minutes on a 401 or 404 from it, and records the gateway's `usage.cost`; `src/lib/usage.ts` prices `typesafe/jev-1.13` too; `scripts/qa/mock-jev.mjs` answers `/decisions` as well.
- `src/lib/derive/config.ts`: `CONTENTS_MODEL` is GLM 5.3, not Flash; SPEC.md §2 says so.
