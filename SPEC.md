...
type DeriveRequest = {
  ...
  sectionId?: string;          // FORMALIZE notes, COMPARE, ANALYZE: where the notes land
};
```
...
4. Route output by destination:
   ...
   - `ANALYZE` → a FIGURE or TABLE block read as data by the vision model: one `PENDING` note on the block (the figure popover's Analyze figure; Analyze table on a selection inside a table)
...
**ANALYZE output contract:** the block's visual attaches as it does for EXPLAIN (`lib/derive/figure.ts`: the image, the SVG source, or the PDF page region); a TABLE block attaches its markup, plus its page region when the PDF has one. The model is the one that reads visuals with the least invention — `DERIVATION_MODEL.ANALYZE`, the most capable model, the same one upload and parse trust — and returns JSON `{kind, summary, structure, readings: [{label, value, certainty: "read" | "estimated"}], data: {columns, rows} | null, takeaway, cautions}`. `read` is only for a value printed on the visual; a value read off an axis by eye is `estimated` and the note marks it with ≈. Validate strictly. The note (`lib/derive/analysis.ts`) carries the summary, the structure, the grid one line per row (notes have no tables), the readings, the takeaway, and the cautions; it lands `PENDING` on the block, anchored to the whole caption or the table selection. Runs behind the heartbeat stream.
...
- **Web access:** the Web toggle beside the scopes (on by default, remembered in the browser) lets Ask search the web through Anthropic's own search tool (`web_search_20260209`, at most five searches per answer; `web: true` on `/api/assistant`). The material stays the first source: the prompt has the model answer from the project, then verify the factual claims against outside sources and add what the project lacks, cite every web source as a markdown link at the point it supports, say plainly when the web contradicts the project, and end with a Web sources list. Sources the model read but did not link are appended under the same heading from the stream's source parts, so every page that shaped the answer is on screen; an outside link opens in a new tab. Never a web result presented as the project's own. Each search records $0.01 on top of the tokens in usage telemetry. The tasks (contradictions, gaps, unsourced), the selection chat, and the media pane's assistant stay off the web.
...
Every add — the add-document dialog's Upload PDF or image, Upload video or audio, Add from Google Drive, and Add URL tabs, and drag-and-drop (PDFs, images, video and audio files) — opens the upload assistant box (`components/reader/upload-assistant.tsx`) before anything is saved. The box states the content type's nuances (formats, caps, what the parse can and cannot read), takes upload instructions, and drives the add itself, one streamed request per page or file, progress in place. Attaching from the library is not an upload and opens no box; a pasted Drive link on a linked account imports directly (§14); the reader's media-figure toast and re-parse keep the floating progress card.
