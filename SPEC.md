...
**Regenerate.** Every AI tool's output can be run again. The reader's cards carry the circular arrow (`RegenerateIcon`), the pages and panels the word Regenerate; the tip says what runs and what it replaces. The run repeats the same derivation on the same input — the card's own selection, the extraction's own question, the answer's own question and range, the matches' own query — never on whatever the input box holds now, so an edited box changes nothing. The new output replaces the old, and only once it is stored: the run persists its own result first and the one it replaced goes then, so a failed run, a stopped one, or a Visualize that declines never loses what stands. Explain, Simplify, Analyze, Visualize, and the moment and page Explains replace their annotation (`DELETE /api/notes/<id>`, no toast — the new annotation is the notice); Match-it replaces its extraction, Extract its distillation, Distill its keypoints, Summarize its summary at that depth, Formalize its article. Ask and Find persist nothing and simply run again. A tool conversation is not regenerated: a turn is sent, never replaced.
