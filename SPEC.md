...
```typescript
type DeriveRequest = {
  type: 'EXPLAIN' | 'SIMPLIFY' | 'SALIENCE' | 'EXTRACT' | 'DISTILL' | 'SUMMARIZE' | 'FORMALIZE' | 'ASK' | 'COMPARE' | 'ANALYZE';
  ...
  notebookId: string;
  anchor?: AnchorInput;        // required for EXPLAIN/SIMPLIFY/EXTRACT/ANALYZE; optional focus for DISTILL
...
- **Left:** document reader. Blocks rendered from DB, selection popover on highlight. **One toolbar per content kind** (`TOOLBARS` in reader-interactions.tsx): the popover shows the tools of the kind under the selection and nothing else, and a tool missing from a kind's list is never offered there. The first tool after the assistant is the kind's lead tool and reads as recommended.
  ...
  | Kind | Blocks | Toolbar |
  |---|---|---|
  | Text | PARAGRAPH, HEADING, LIST, CODE | Assistant, Explain, Simplify, Extract, Comment, Link across texts, highlight colors, Add to notes, Read aloud |
  ...
  Analyze (§4) exists only on the table and figure toolbars; Simplify, Extract, and Read aloud only on text. A new kind of content gets a new row here and a new list in `TOOLBARS`, never a condition on a button.
...
- **Reader views** (`reader-panes.tsx`; the button at the bottom left of the reader): Normal shows one document; Side by Side and Top and Bottom show two panes, each with the full tool set, the view and the second document carried in the URL (`?view=side|stack&doc2=`); a fresh open is Normal. A split view is its own layout, so two panes never crowd each other:
  - Each pane has a **pane header** — one row at its top, above the scroller, never over the text: the pane's document (a select; choosing changes the URL), the article menu and the search, and Distill. In Normal view the article menu floats over the top of the page and hides once the reader scrolls; in a split view it lives in the header and stays in reach, its panels dropping below the header.
...
- No pill, tab, select, or collapsed row cuts its text with an ellipsis. A label is written to fit (the Add-document tabs: PDF or image, Video or audio, Google Drive, URL, Library); a document title in the top bar's document pill, the document list, or a pane's document select is cut at a word boundary, the full title one hover away (`clipWords`, `lib/markdown-preview.ts`); a collapsed note or annotation shows its gist.
- The text under the open selection popover keeps a selection tint (a mark of kind `selection`, the same color as the browser's selection): the browser's own selection goes the moment the assistant's command box or the comment box takes focus, and while the assistant runs, but the tint stays until the popover closes. The native selection over that mark paints nothing, so the two never stack. Reading mode only — edit mode's blocks are the browser's editable regions.
...
- **Admin financials page** (`/admin/financials`, admin-gated): MRR and subscribers from `Subscription`, revenue, refunds, and failed payments from `Payment`, beside the usage page's AI cost — margin is revenue less that cost (§20). Unit economics: accounts with AI use in 30 days and the AI cost per such account, and the AI cost per call by function — the numbers the tier prices are set against.
...
Every add — the add-document dialog's Add URL, Upload PDF or image, Upload video or audio, and Add from Google Drive tabs (in that order; the dialog opens on Add URL), and drag-and-drop (PDFs, images, video and audio files) — opens the upload assistant box (`components/reader/upload-assistant.tsx`) before anything is saved. The box states the content type's nuances (formats, caps, what the parse can and cannot read), takes upload instructions, and drives the add itself, one streamed request per page or file, progress in place. Attaching from the library is not an upload and opens no box; a pasted Drive link on a linked account imports directly (§14); the reader's media-figure toast and re-parse keep the floating progress card.
...
**Tiers (TIERS.md).** Unitos Free drops images up to 5 MB; above that the account needs Unitos Premium. No tier stores an image past 25 MB. The client checks the size before the upload so the refusal is instant, and the route checks it again — the client's check is a courtesy, not the gate.
...
- **The flag:** `User.tier` (`FREE` | `PREMIUM` | `ULTRA`, default `FREE`; `lib/tiers.ts`, `hasPremium` is the check every Premium gate makes — Ultra holds everything Premium holds). Stripe sets it through the webhook (§20); the admin never does. The single local reader (sign-in off) is Ultra: there is no account to gate. Settings shows the state under Unitos Premium.
...
Three tiers (TIERS.md): Unitos Free, Unitos Premium, Unitos Ultra. Stripe holds the subscriptions; the app mirrors them. The pricing section in Settings is the one reader-facing surface (below); the beta notice (§2) still says every beta account gets Unitos free and unlimited, which needs a decision once billing is live — the two say different things to the same account.

- **Configuration:** `STRIPE_SECRET_KEY` turns billing on; `STRIPE_WEBHOOK_SECRET` verifies the webhook; `STRIPE_PRICE_PREMIUM_MONTHLY`, `STRIPE_PRICE_PREMIUM_YEARLY`, `STRIPE_PRICE_ULTRA_MONTHLY`, `STRIPE_PRICE_ULTRA_YEARLY` are the Stripe price ids a checkout sells (`lib/billing/stripe.ts`). The admin's services list shows which are set. Unset, every billing route answers 503 and the financials page says so.
- **The tier:** `User.tier`, recomputed after every subscription event as the highest tier among the account's subscriptions whose Stripe status grants access (`active`, `trialing`, `past_due` — Stripe retries the card during `past_due`; `canceled`, `unpaid`, `incomplete`, `paused` do not). `lib/tiers.ts` is the one place tiers are compared: `hasPremium(tier)` is every Premium gate (offline work §17, large images §16); Ultra holds everything Premium holds. An Ultra-only gate asks `tierAtLeast(tier, "ULTRA")`. The admin never sets a tier (§18).
- **Checkout:** `POST /api/billing/checkout {tier, interval}` (signed-in account; `PREMIUM` | `ULTRA`, `month` | `year`) creates the account's Stripe customer on first use (`User.stripeCustomerId`, `ensureCustomer`) and answers the hosted Checkout URL, `mode: subscription`, `userId` and `tier` in the subscription's metadata, promotion codes allowed; success returns to `/settings?billing=done`, cancel to `/settings`. `POST /api/billing/portal` answers the customer portal URL (change the card, switch the interval, cancel). `GET /api/billing` answers the account's tier, whether billing is open, and its access-granting subscription.
- **Webhook:** `POST /api/stripe/webhook`, a public door in the middleware (Stripe signs the raw body; the route checks the signature before reading anything). Events: `checkout.session.completed` (the customer id lands on the account, the new subscription syncs at once), `customer.subscription.created` / `updated` / `deleted` (upsert `Subscription` by Stripe's id, recompute the tier), `invoice.paid` / `invoice.payment_failed` (upsert `Payment` kind `invoice` by invoice id: `amount_paid` when paid, `amount_due` when failed, the line's period), `charge.refunded` (every refund on the charge as its own `Payment` kind `refund`, negative). Every handler upserts by Stripe's id, so a redelivered or out-of-order event lands on the same row. `StripeEvent` records each event id after its handler finishes: a redelivery is answered 200 and skipped; a handler that throws answers 500 so Stripe retries. Subscribe the endpoint to exactly these event types in the Stripe dashboard (`HANDLED_EVENTS` in `lib/billing/sync.ts`).
- **Reset account (§7):** the reset leaves the tier to the subscriptions — `recomputeTier` runs after it — so a paid tier survives a reset and an operator-set tier does not.
- **Financials:** the admin page (§7). MRR counts every access-granting subscription at its monthly rate (a yearly price at a twelfth). Revenue is `Payment` rows `paid` and `refunded` summed (refunds negative). AI cost is `UsageEvent` (§7), estimates from list prices; margin is revenue less that cost, both read as dollars.
- **Settings pricing section:** `components/settings-form.tsx`, signed-in accounts only. `GET /api/billing` answers the account's tier, its access-granting subscription, and the live Stripe prices (`lib/billing/prices.ts`: read straight from Stripe, a five-minute in-process cache, never a hardcoded number — a price edited in the Stripe dashboard shows here with no redeploy). On `FREE`, a Monthly/Yearly toggle sits over the two tier cards; the Yearly button carries the largest saving among the tiers as a badge, and a tier whose yearly price is cheaper than twelve months at its monthly price shows its own saving on its card (`Math.floor`, never overstates). Upgrade posts `/api/billing/checkout` and follows the returned URL. On `PREMIUM` or `ULTRA`, the section shows the plan, its price, and its renewal or cancellation date, with Manage billing opening the customer portal. Unset Stripe keys, or the local reader (no account), fall back to the plain Unitos Premium status line this section replaces.
