...
- **Surface:** "Add from Google Drive" is a tab in the add-document dialog, beside Upload PDF and Upload video — same progress card, same one-at-a-time ingest for multiple picks. Drive is the one add that skips the upload assistant box (§15): the server fetches the file at import time, so the sandbox review has nothing to read.

---

## 15. The upload assistant

Every add — the add-document dialog's Upload PDF, Upload video or audio, and Add URL tabs, and drag-and-drop — opens the upload assistant box (`components/reader/upload-assistant.tsx`) before anything is saved. The box states the content type's nuances (formats, caps, what the parse can and cannot read), takes upload instructions, and drives the add itself, one streamed request per page or file, progress in place. Attaching from the library is not an upload and opens no box; adding from Google Drive imports server-side and opens no box (§14); the reader's media-figure toast and re-parse keep the floating progress card.
