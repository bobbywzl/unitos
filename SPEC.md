...
- Tooltips: every control in the workspace header, the rail, the reader's toolbars, the article menu, and the selection popover carries a one-line caption of what it is and does. It opens beside the control on mouse hover (after a short delay; at once when moving straight from one control to the next) or on keyboard focus, away from the text, and hides on a press, Escape, or a scroll. One shared layer renders them (`components/tooltip.tsx`, `data-tooltip` and `data-tooltip-side` on the control); these controls never use the native `title` attribute. Touch shows none.
- **Welcome flow (first visit):** on the dashboard of an account with no project yet, a screen fades in — "Welcome to your all-powerful notebook." over the mark covering the whole background — and fades out into the dashboard (`components/works/welcome-flow.tsx`; localStorage keeps it to one showing). Then the first-steps card explains the functions: start a new project (the directive), add documents, the AI tools, and the ? at the top right of a project. That ? button carries a pulsing dot until the guide is opened once; the guide leads with an emphasized Circle & ask section (figures, and handwritten pages where the whole PDF is a figure — lasso highlight included). The /signin backdrop carries the same mark, dimmed, covering the page's top-left quadrant.
