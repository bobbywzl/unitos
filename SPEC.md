...
- One note structure, everywhere a note renders (the tray, the notes full page, a compare pane): a header row — collapse chevron and the note's id at the left (`#` plus the last six characters of its cuid; click copies it; searching `#k3x9pq` finds the note), jump, pin, and select at the right — then the body, then the actions. The editor keeps that shape: the header stays, the textarea grows with the text at the body's size and line height (`.note-text`), and a newline typed in the editor stays a line break on display (markdown alone folds single newlines into spaces), so the note looks the same after Done as it did while editing.
