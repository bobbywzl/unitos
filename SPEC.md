...
- **AI:** Kimi K3 (Moonshot AI, `kimi-k3`) via the Vercel AI SDK (`ai` package, `@ai-sdk/moonshotai` provider), streaming responses. One model behind every feature (`lib/kimi.ts` is the client); the model stays a per-derivation-type config constant, with a reasoning effort beside it (`DERIVATION_EFFORT` in `lib/derive/config.ts`: `high` for the reader's tools, `max` for ANALYZE). Upload and parse — the upload assistant's review and instruction check, the URL core and structure passes, Import PDF's judgment, and conversion — run on the same model (`PARSE_MODEL`): what the parse gets wrong, every later tool inherits. The key is `KIMI_API_KEY` (or `MOONSHOT_API_KEY`); `KIMI_BASE_URL` overrides the endpoint.
