...
const deriveSchema = z
  ...
  video: z
    ...
    .optional(),
  // EXPLAIN on a handwritten page (SPEC.md §14): Circle & ask. The PAGE block,
...
export async function POST(req: Request) {
  ...
  // A video or page anchor stands in for a text anchor on EXPLAIN (SPEC.md §11, §14).
  if (
  ...
  // EXPLAIN on a handwritten page (SPEC.md §14): Circle & ask. The server
  ...
  if (data.type === "EXPLAIN" || data.type === "SIMPLIFY" || data.type === "SUMMARIZE") {
    ...
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        // stays marked on the page (SPEC.md §14).
