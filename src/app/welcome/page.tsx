...
// The welcome page: the confirmation link lands here. One job — set a
// password so the account can sign in next time. Accounts that already have
// one (or arrive by other means) go straight to the app.
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-md flex-1 flex-col justify-center px-6 py-16">
        <div className="rise-in">
          ...
          <h1 className="font-display text-[2rem] leading-[1.1] text-ink sm:text-[2.4rem]">
            {t("signin.welcomeTitle", { name: user.name })}
          </h1>
          <p className="mt-3 text-sm leading-relaxed text-sand-600">
            {t("signin.welcomeDesc", { email: user.email })}
          </p>
          ...
          >
            ...
            <p className="text-center text-xs text-sand-600">
              <Link href="/" className="font-semibold text-clay hover:brightness-110">
                {t("signin.welcomeSkip")}
              </Link>
            </p>
          </form>

          <p className="mt-5 text-xs leading-relaxed text-sand-600">{t("signin.welcomeNext")}</p>
        </div>
      </main>
    </div>
  );
