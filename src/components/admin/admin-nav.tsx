"use client";

import Link from "next/link";
import { useRouter } from "next/navigation";
import { useT } from "@/components/lang-provider";

// Shared header of the admin pages (Scalae admin pattern): one tab per page,
// App link, sign out.
type Tab = "feedback" | "digest" | "usage" | "gateway" | "notifications" | "accounts" | "clicks" | "funnel" | "billing";

export function AdminNav({ active }: { active: Tab }) {
  const router = useRouter();
  const t = useT();

  async function signOut() {
    await fetch("/api/admin/auth", { method: "DELETE" });
    router.push("/admin/login");
  }

  const tab = (href: string, id: Tab, label: string) => (
    <Link
      href={href}
      className={`rounded-full px-3 py-1 text-xs font-semibold ${
        active === id ? "bg-ink text-paper" : "bg-card text-sand-600 shadow-soft hover:text-clay-800"
      }`}
    >
      {label}
    </Link>
  );

  return (
    <nav className="mb-6 flex items-center gap-1">
      {tab("/admin", "feedback", t("admin.feedback"))}
      {tab("/admin/digest", "digest", t("admin.digest"))}
      {tab("/admin/usage", "usage", t("admin.usage"))}
      {tab("/admin/gateway", "gateway", t("admin.gateway"))}
      {tab("/admin/notifications", "notifications", t("admin.notifications"))}
      {tab("/admin/accounts", "accounts", t("admin.accounts"))}
      {tab("/admin/clicks", "clicks", t("admin.clicks"))}
      {tab("/admin/funnel", "funnel", t("admin.funnel"))}
      {tab("/admin/billing", "billing", t("admin.billing"))}
      <div className="ml-auto flex items-center gap-3 text-sm">
        <Link href="/" className="text-sand-600 hover:text-clay-700">
          {t("common.app")}
        </Link>
        <button onClick={() => void signOut()} className="text-sand-600 hover:text-clay-700">
          {t("common.signOut")}
        </button>
      </div>
    </nav>
  );
}
