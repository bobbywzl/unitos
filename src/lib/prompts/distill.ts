...
export function corpusDistillPrompt(ctx: {
  ...
  return [
    profileLines(ctx.profile),
    "",
    "The reader wants their whole corpus distilled against one question. Every document is above, each starting with [document <id>] and its title, its blocks each starting with [block <id>].",
    "",
    `Question: ${ctx.question}`,
    "",
    "Scan every document and pull the quotes that answer the question.",
    "1. quotes: 3 to 14 verbatim spans — the most important sentences and paragraphs, from anywhere in the corpus. Video transcripts count like any text.",
    "2. Each span is one contiguous character range inside one block: a full sentence up to a full paragraph.",
