...
export function distillPrompt(ctx: PromptCtx): string {
  return [
    ...
    "3. quote: the span copied out verbatim — the block's own characters, its own punctuation, nothing added, nothing elided.",
    "4. start and end are character offsets into that block's text: 0 is the first character of the block's text, not of the document, and the [block <id>] marker line is not part of the text it heads. Use block ids exactly as they appear in [block <id>] markers. When the offsets and the quote disagree, the quote is what counts.",
    "5. Every quote answers the question in a different way: a different facet, a different mechanism, a different piece of evidence, or a different side. Before adding a quote, check the quotes already chosen. If one of them already says the same core point, drop the new one, or keep only the stronger of the two. Never pull two quotes that answer the question the same way.",
    "6. Order quotes as they appear in the document.",
    ...
    "8. Fewer, stronger quotes beat many weak ones. Skip anything that does not bear on the question. Skip anything that only repeats a point another quote already makes.",
    "9. When the document does not answer the question, return the one or two quotes closest to it, and say in each caption what the quote does and does not answer. Never stretch a quote into an answer it does not give.",
    "",
    'Return ONLY JSON: {"quotes": [{"blockId": "<id>", "start": 0, "end": 42, "quote": "<verbatim span>", "caption": "<text>"}, ...]}',
  ].join("\n");
}
...
export function corpusDistillPrompt(ctx: {
  ...
  return [
    ...
    "2. Each span is one contiguous character range inside one block. Cut it to what answers the question: a phrase, one sentence, several sentences, or a whole paragraph. Do not pad a phrase out to its sentence, and do not cut a paragraph that answers as one argument.",
    "3. quote: the span copied out verbatim — the block's own characters, its own punctuation, nothing added, nothing elided.",
    "4. start and end are character offsets into that block's text: 0 is the first character of the block's text, not of the document, and the [block <id>] marker line is not part of the text it heads. Use block ids exactly as they appear in [block <id>] markers — block ids are unique across all documents. When the offsets and the quote disagree, the quote is what counts.",
    ...
    "6. Where documents answer together — agree, disagree, extend each other — pull from each, so the answer spans the project, not one document.",
    "7. Order quotes by document as listed, then by position.",
    `8. caption: one sentence per quote, two at most, in ${languageName(ctx.lang)}. Plain words, no filler. State the answer the quote gives, with its number or its named mechanism, and how it sits against the other documents: agrees, disagrees, extends. If the quote answers only one facet of the question, name that facet and say the quote answers that facet only. A caption must stand on its own: name the subject, never write "the question" or "this quote".`,
    "9. Fewer, stronger quotes beat many weak ones. Skip documents with nothing to say. Skip anything that only repeats a point another quote already makes.",
    "",
    'Return ONLY JSON: {"quotes": [{"blockId": "<id>", "start": 0, "end": 42, "quote": "<verbatim span>", "caption": "<text>"}, ...]}',
  ].join("\n");
}
