...
// document built from them, or both. The route resolves every block id and
// every quote against the real block text before anything is stored: a
// quote the model did not copy verbatim falls back to its whole block, and
// a block id that names no block drops (SPEC.md §4).
...
function memberList(members: { id: string; title: string }[]): string {
  return members.map((m) => `[document ${m.id}] "${m.title}"`).join(", ");
}
...
export function stitchSelectPrompt(ctx: StitchSelectCtx): string {
  return [
    ...
    "",
    `The documents above are the members of a multi upload, each under its id: ${memberList(ctx.members)}. Every block is tagged [block <id>]; block ids are unique across all members.`,
    "",
...
export function stitchPrompt(ctx: StitchCtx): string {
  ...
  return [
    ...
    `3. reply: the answer to the command in plain sentences, in ${name}. A question gets its answer here — concise, concrete, every claim with the member it comes from named — and a page only when the command asks for one. A command to gather, link, or write gets one to three sentences on what the page holds, how many links, or why the command could not be done with these members. Never restate the page.`,
