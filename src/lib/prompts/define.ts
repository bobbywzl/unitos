...
// selection is one word, never Chinese text (SPEC.md §6). The definition
// streams into the toolbar under the row and persists nothing (SPEC.md §4).
// What makes it worth reading: the reader knows what the word means in this
...
export function definePrompt(ctx: PromptCtx): string {
  return [
    ...
    "",
    `The reader selected a word in "${ctx.documentTitle}". The full document is above.`,
    "",
    ...
    "",
    "Define the selected word for this reader.",
    ...
    'Keep it under 40 words. Plain text: no markdown, no heading, no list. Start with the meaning: no preamble, never repeat the word, never open with "In this context" or "Here".',
