// Import PDF classification (SPEC.md §14): the attached images are sample
...
// pages). Runs only when text extraction yielded little — a PDF with a real
// text layer is an article without asking.
export function classifyPrompt(params: { pageCount: number; textChars: number }): string {
  return [
    ...
    '- "handwritten": rough handwritten notes, drawings, sketches, whiteboard or notebook photos, or scanned pages whose content the text layer missed.',
