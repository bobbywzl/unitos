...
3. Leave `MOONSHOT_API_KEY` unset. Without a key the URL ingest skips the core and structure passes and the PDF judgment falls back to the text-layer yield, so the loop tests the mechanical parser — the layer fixes land in. Set the key only for a round about page chrome, which those passes remove.
