...
2. Copy `.env.example` to `.env` and fill in:
   ...
   - `TYPESAFE_API_KEY` — Jev, TypeSafe AI's decision model: the lead tool of the selection toolbar, Stitch's route and select passes, and the nudges a reader has already earned; unset, those run without it (`TYPESAFE_BASE_URL` overrides the endpoint)
