...
- Google, Apple, and email sign-in at `/signin` (dual mode: with `SESSION_SECRET` plus any provider's credentials the app is gated; without, it runs as a single local reader). Email sign-up takes the email alone, creates the account only when the confirmation link is clicked, and lands on the dashboard; the card comes after 30 minutes of active time (the billing ask, SPEC.md §24). Returning users sign in with the same email link, or with email + password once Forgot password has set one. Projects, context, and digests belong to the signed-in account; the first account to sign in adopts the local reader's data
