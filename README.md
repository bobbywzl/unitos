...
- GLM 5.3 and GLM 5.3 Flash (Z.ai) through the AI gateway for the reader's tools, the assistant, Stitch, and the readings; Kimi K3 (Moonshot AI) for calls that carry an image and the assistant with Web on, and for every GLM call without the gateway; all via the AI SDK, streaming, automatic prompt caching (the parsed document — and the digest at assistant scopes — is the cached prefix); Claude Fable 5.1 (Anthropic) at high effort for the parse passes — the URL core, structure, and layout passes — Claude Opus 5 for Import PDF's judgment, conversion, and Visualize
