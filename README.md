...
- Derivations via one pipeline (`/api/derive`): EXPLAIN (annotation rail), SIMPLIFY (inline swap, revert on click), SALIENCE (toggleable overlay), EXTRACT (pending note with sources), COMPARE (two documents → one pending note of agreements, disagreements, and what only one covers, sourced on both; from the document list's menu), ANALYZE (a figure or table read as data by the vision model — printed values verbatim, estimates marked with ≈, the grid row by row, cautions — as one pending note on the block; it leads the figure and table toolbars and never appears on text), ASK (a question about a time range of a video or audio document, answered from the transcript in that range; Add to notes lands it pending with a time source)
