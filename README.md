...
- One toolbar per content kind: select one word or one phrase in any document — an article, a PDF, a transcript, slides, a sheet, a blank document — and Define is the first row, under the highlight colors: the meaning the word has in its sentence, in plain words (a key term's is the glossary's); select text for the Assistant, Simplify, Visualize, Comment, Link, highlight, Add to notes, and Read aloud; select in a table or circle it for Table tools; circle a figure for Figure tools; circle an equation for Equation tools. Each kind offers its own tools and nothing else
...
- Assistant panel with two scopes — Project (this project whole) and Projects (every project whole) — plus contradiction, gap, and unsourced checks as clickable cards. With Web on (the default), Ask searches the web — Z.ai's Web Search API under GLM 5.3, Moonshot's web-search tool under Kimi K3 — to verify its answer against outside sources and cites every page it used as a link, ending with a Web sources list; the project stays the first source
...
- Google, Apple, and email sign-in at `/signin` (dual mode: with `SESSION_SECRET` plus any provider's credentials the app is gated; without, it runs as a single local reader). Email sign-in takes a name and an email, creates the account only when the confirmation link is clicked, and lands on the dashboard; the card comes after 30 minutes of active time (the billing ask, SPEC.md §24). Returning users sign in with the same email link, or with email + password once Forgot password has set one. Projects, context, and digests belong to the signed-in account; the first account to sign in adopts the local reader's data
...
- The onboarding funnel: how far new visitors get from the sign-in page to a subscription (sign-in page, account, dashboard, reader, plans, order, checkout, subscribed), one browser joined to the account it makes; the admin funnel page (`/admin/funnel`) shows the visitors that reached each step, where each stopped, new visitors by day, and the latest visitors
...
- GLM 5.3 and GLM 5.3 Flash (Z.ai) through the AI gateway for the reader's tools, the assistant, Stitch, and the readings; Kimi K3 (Moonshot AI) for Extract, for calls that carry an image, and for every GLM call without the gateway; all via the AI SDK, streaming, automatic prompt caching (the parsed document — and the digest at assistant scopes — is the cached prefix); Claude Opus 5.5 (Anthropic) at max effort for the parse passes — the URL core, structure, and layout passes — and for Import PDF's judgment, conversion, Visualize, and every reading of an SVG chart; the gateway page's Model per function section (/admin/gateway) sets another model for any function, probed with one call before it is saved
...
2. Copy `.env.example` to `.env` and fill in:
   ...
   - `TYPESAFE_API_KEY` — Jev, TypeSafe AI's decision model (`lib/jev.ts`): the lead tool of the selection toolbar, Stitch's route and select passes, the nudges a reader has already earned, the figure audit's caption check, the sheet header check, the URL import's wall check, the digest's ranked cut, the tool checks, the History panel's small edits, and a recording's chapters; unset, all of those run without it. With the gateway, `TYPESAFE_API_KEY` lives on the gateway host and the app calls its `/typesafe/systemone` pass-through (OpenRouter serves Jev there); without it, an OpenRouter key here with `TYPESAFE_BASE_URL=https://openrouter.ai/api/v1`. Either way `TYPESAFE_MODEL` on the app's host names the model (`~typesafe/jev-latest` or `typesafe/jev-1.13` at OpenRouter, `jev-latest` at TypeSafe)
