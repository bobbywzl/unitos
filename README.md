...
- Video transcription starts on its own when the video is added, via a provider ladder — YouTube: Gemini (`GEMINI_API_KEY`), then YouTube captions through the player API, then the watch page; uploads: Whisper (`OPENAI_API_KEY`), then Gemini — with read-along highlight and click-to-seek; Find searches the transcript and answers with seekable time ranges; Explain reads the circled frame plus the transcript (on YouTube, Gemini watches the clip) and saves the explanation as an annotation at that moment
