...
- Contents: the article's parts at the top left of the reader, built by AI the first time they are opened (headings when the document has them, segments by what the text does when it does not), each a jump to where the part starts; the button hides once the reader scrolls
...
- GLM 5.3 and GLM 5.3 Flash (Z.ai) through the AI gateway for the reader's tools, the assistant, Stitch, the readings, and the parse passes; Kimi K3 (Moonshot AI) for calls that carry an image and the assistant with Web on, and for every GLM call without the gateway; all via the AI SDK, streaming, automatic prompt caching (the parsed document — and the digest at assistant scopes — is the cached prefix); Claude Fable 5.1 (Anthropic) at its highest reasoning effort for the import — upload review, the URL core and structure passes, Import PDF's judgment, and conversion — and for Visualize
